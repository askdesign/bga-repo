...
//Adds Page Content
add_action( 'genesis_before_loop', 'optimal_do_portfolio_content' );
function optimal_do_portfolio_content() {
...
add_action( 'genesis_entry_content', 'optimal_portfolio_do_post_content' );
function optimal_portfolio_do_post_content() {
    
    if ( genesis_get_option( 'optimal_portfolio_content' ) == 'excerpts' ) {
        the_excerpt();
    
    } else {
        if ( genesis_get_option( 'optimal_portfolio_content_archive_limit' ) )
